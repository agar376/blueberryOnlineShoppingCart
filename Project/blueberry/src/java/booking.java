...
import java.util.*;
@WebServlet(name = "booking", urlPatterns = {"/bh"})
public class booking extends HttpServlet implements HttpSessionListener {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String details=response.encodeRedirectURL("details");
...
out.println("<div style='display: block; text-align: center; padding: 0px 160px 5px 160px'>  ");
            ...
            out.println("Welcome "+user+"<br><h1>Booking History</h1>");
        try {
            Class.forName("oracle.jdbc.driver.OracleDriver");
            Connection con=DriverManager.getConnection("jdbc:oracle:thin:@127.0.0.1:1521:xe","system","shobhit");
                Statement stmt=con.createStatement();
            ResultSet rs=stmt.executeQuery("select booking_code,booking_date,status from booking_history where username='"+user+"'");
            out.println("<form method='post' action='"+details+"'>");
