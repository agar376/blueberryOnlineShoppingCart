...
public class cart extends HttpServlet implements HttpSessionListener
{
...
out.println("Welcome "+user+"<br><h1>Shopping Cart</h1>");
        ...
        while(rs.next())
        {i++;
            String q=(request.getParameter("qty"+i));
            
            if(!(q.isEmpty()))
            {
                cart.add(new shopping_item(Integer.parseInt(rs.getString("itemcode")),rs.getString("name"),Double.parseDouble(rs.getString("itemprice")),Integer.parseInt(q)));
            }
        }
